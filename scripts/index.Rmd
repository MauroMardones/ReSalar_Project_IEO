...
s_normalgs = select(filtered_db$size, ncomp = 2:6)
plot(s_normalgs)
s_normalei = select(obj_id$EI$length_total_cm, ncomp = 2:6)
plot(s_normalei)
s_normalbs = select(obj_id$BS$length_total_cm, ncomp = 2:6)
plot(s_normalbs)
s_normalsswi = select(obj_id$SSIW$length_total_cm, ncomp = 2:6)
plot(s_normalsswi)

# fit a Normal mixture model
modgs = mixfit(filtered_db$size, 
               ncomp = 4, 
               pi = c(0.25, 0.25, 0.25, 0.25),  # Pesos iniciales
              mu = c( 20, 30, 40, 50),      # Medias iniciales diferentes
               sd = c(5, 5, 5, 5))    # Desviaciones estándar iniciales más grandes
...
medias <- seq(20, 70, length.out = ncomp)

# Definir pesos iniciales (uniformes)
pesos <- rep(1/ncomp, ncomp)  # Todos los pesos iguales (puedes ajustar si es necesario)

# Definir desviaciones estándar iniciales (puedes ajustarlas según tus datos)
desviaciones <- rep(1, ncomp)  # Desviaciones estándar iguales (ajusta si es necesario)
...
# run ELEFAN with simulated annealing
res_SAgs <- ELEFAN_SA(lfq_results$GS, 
                    SA_time = 60*0.5, 
                    MA = 5, 
                    agemax = 5,
                    seasonalised = TRUE, addl.sqrt = TRUE,
...
# run ELEFAN with genetic algorithm
res_GAgs <- ELEFAN_GA(lfq_results$GS, 
                    MA = 5, 
                    seasonalised = TRUE, 
                    maxiter = 10, 
                    agemax = 5,
...
# run ELEFAN with simulated annealing
res_SAgshe <- ELEFAN_SA(lfq_resultshe$GS, 
                    SA_time = 60*0.5, 
                    MA = 5, 
                    agemax = 5,
                    seasonalised = TRUE, addl.sqrt = TRUE,
...
# run ELEFAN with genetic algorithm
res_GAgshe <- ELEFAN_GA(lfq_resultshe$GS, 
                    MA = 5, 
                    seasonalised = TRUE, 
                    maxiter = 10, 
                    agemax = 5,
